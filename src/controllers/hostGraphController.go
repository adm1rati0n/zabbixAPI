...
func getGraphData(w http.ResponseWriter, r *http.Request) {
	...
	var hostsInfo []models.HostGraph
	for rows.Next() {
		var hostInfo models.HostGraph
	...
	for rows.Next() {
		...
		err = rows.Scan(&host.Hostid, &host.Name_host, &host.Work_calculation)
		if err != nil {
			panic(err)
		}
		hosts = append(hosts, host)
	}
	var hostMap models.HostMapGraph
	hostMap.Hosts = hosts
	hostMap.HostsInfo = hostsInfo
	dir, _ := os.Getwd()
	tmpl := template.Must(template.ParseFiles(dir + "/src/template/hostGraphTemplate.html"))
